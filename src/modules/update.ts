...
export const getAndUpdate = async (): Promise<void> => {
  const valueSecs = document.querySelector('.section')!.querySelectorAll('.value');
  ...
  try {
    ...
    for (const sec of valueSecs) {
      ...
      switch (sec.id) {
        case 'cityLoc':
          if (sec.textContent === '') { // If this is the 1st Iteration
            sec.classList.toggle('hide');
            sec.textContent = `${retreivedData.loc}`;
          }else { // If this is 2nd & onward
            sec.classList.toggle('hide');
            setTimeout(() => {
              sec.textContent = `${retreivedData.loc}`;
              sec.classList.toggle('hide');
